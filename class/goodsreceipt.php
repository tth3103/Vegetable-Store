<?php
    ...
    class goodsreceipt extends DBConnection{
        ...
        public function getByID($GoodsReceiptID){
            $sql = "SELECT * 
                    FROM ((((`goodsreceipt`
                    INNER JOIN `goodsreceiptdetails` ON goodsreceiptdetails.GoodsReceiptID = goodsreceipt.GoodsReceiptID)                               INNER JOIN `vegetable` ON vegetable.VegetableID = goodsreceiptdetails.VegetableID)
                    INNER JOIN `staff` ON goodsreceipt.StaffID = staff.StaffID)
                    INNER JOIN `supplier` ON goodsreceipt.SupplierID = supplier.SupplierID)
